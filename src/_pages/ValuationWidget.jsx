...
const ValuationWidget = ({ embedded = false }) => {
    ...
    const zoekWoning = async () => {
        ...
        try {
            ...
            if (res.found) {
                ...
                let detectedCity = s1.city;
                if (AMSTERDAM_PLAATSEN.test(wp)) detectedCity = 'amsterdam';
                else if (UTRECHT_PLAATSEN.test(wp)) detectedCity = 'utrecht';
                const detectedWijk =
                    res.lat != null && res.lon != null ? await wijkForPoint(res.lat, res.lon, detectedCity) : null;
                setS1((prev) => {
                    const next = { ...prev, city: detectedCity };
                    ...
                    if (res.woningtype && !prev.type) next.type = res.woningtype;
                    if (detectedWijk && !prev.wijk) next.wijk = detectedWijk;
                    return next;
                });
            }
        } catch {
    ...
    useEffect(() => {
        if (!/\d/.test(s1.adres) || s1.adres.trim().length < 5) return;
        if (lookupTimer.current) clearTimeout(lookupTimer.current);
        lookupTimer.current = setTimeout(() => { zoekWoning(); }, 800);
        return () => { if (lookupTimer.current) clearTimeout(lookupTimer.current); };
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [s1.adres, s1.postcode]);
    ...
    const embeddedTitle = isNl ? 'Bereken de waarde van je woning' : 'Calculate the value of your home';
    const embeddedSubtitle = isNl ? 'Gratis indicatie op basis van het Kadaster en actuele marktdata.' : 'Free estimate based on the Land Registry and current market data.';
        ...
        <>
            {embedded && !result && (
                <div className={styles.embeddedHeader}>
                    <h2 className={styles.embeddedTitle}>{embeddedTitle}</h2>
                    <p className={styles.embeddedSubtitle}>{embeddedSubtitle}</p>
                ...
                <div className={styles.cardContent}>
                    ...
                    {!result && !embedded && (
                        <>
                            ...
                            <h2 className={styles.widgetTitle}>{t.title}</h2>
                            <p className={styles.stepLabel}>{t.step} {step} {t.stepOf} 3</p>
                            <div className={styles.progressBarTrack}>
                                <div className={styles.progressBarFill} style={{ width: `${progress}%` }} />
                            </div>
