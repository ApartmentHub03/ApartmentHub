-- Migration: Fix thank_you_for_offer trigger — d.created_at does not exist
--
-- The thank_you_for_offer() trigger function (migrations 20260718160000 and
-- 20260718200000) references d.created_at in its "first upload post-viewing"
-- check. The live documenten table has no created_at column — only
-- uploaded_at (see supabase/schema/dossiers-schema.sql:46-54). The original
-- 20251130 migration created documenten WITH created_at, but the canonical
-- schema shows the table was recreated without it.
--
-- This was latent because the original upload-flow bug meant zero documenten
-- rows were ever written, so the trigger never fired. Now that the upload
-- flow correctly inserts rows, the trigger fires and raises:
--   ERROR: column d.created_at does not exist
--
-- Fix: replace d.created_at with d.uploaded_at (same semantic — when the doc
-- was uploaded). This mirrors the sync_accounts_from_dossier_data() trigger
-- (migration 20260224000000 line 91), which already uses d.uploaded_at.
--
...
BEGIN
    ...
    -- persoon_id with uploaded_at > viewing_start_time. If more than 1, this
    -- isn't the first — skip. (The recompute trigger fires AFTER INSERT so
    -- the row we're processing is already in the table, hence > 1.)
    -- NOTE: uses uploaded_at, not created_at — the documenten table has no
    -- created_at column (only uploaded_at).
    ...
    WHERE d.persoon_id = NEW.persoon_id
      ...
      AND d.uploaded_at > v_viewing_start_time;
...
'Trigger function: fires when a candidate uploads their first document after attending a viewing. Sends n8n webhook so Zoko can send the thank_you_for_making_the_offer template. Guarded by thank_you_offer_sent table to prevent duplicate fires. Uses uploaded_at (not created_at — the documenten table has no created_at column).';