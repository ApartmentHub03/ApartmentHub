...
// /sell-intake is served as a raw Response — the portal HTML is a complete
...
//
// Auth gate:
//   - No session  → 302 to /sell-intake/login (the React OTP form)
...
async function loadPortalHtml(): Promise<string> {
  ...
  const p = path.join(process.cwd(), "src/app/sell-intake/_portal/index.html");
...
export async function GET(req: NextRequest) {
  ...
  if (!session) {
    const url = new URL("/sell-intake/login", req.url);
    // Preserve any next= the user came in with so they land back on /sell-intake.
  ...
  try {
    ...
    return new Response(
      "<!doctype html><html><body><h1>Portal not found</h1>" +
        "<p>Expected: <code>src/app/sell-intake/_portal/index.html</code></p></body></html>",
